...
> - Skip-grill — go straight to synthesis (Other → type `skip-grill` or `synthesize-now`)
...
| `skip-grill` / `synthesize-now` | Abandon all questions, go straight to synthesis | THINK review checkpoint only |
...
> 你想怎么继续？
> - 就用这 5 个问题（推荐）
> - drop 某几个（Other → 输入 ID，比如 `drop Q2, Q5`）
> - 加一个问题（Other → 输入你的问题）
> - Pivot —— 换个角度重新生成（Other → 可以描述新角度）
> - Skip-grill —— 直接跳过问题阶段去综合（Other → 输入 `skip-grill` 或 `synthesize-now`）
...
- 答完后 Claude *必须*问你为什么选这个（开放文本）—— Socratic 的开放性活在这里
...
| `skip-grill` / `synthesize-now` | 抛弃所有问题，直接跳到综合 | 仅 THINK review checkpoint |
